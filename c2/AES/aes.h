#ifndef AES_H
#define AES_H

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define KSIZE 	16 				// key size
#define KROUND 	44				// round key size
#define SUB(a) S_Box[a]			// S-box substitution
#define a(r, c) a[r * 4 + c]	// get entry (r, c) in key
#define MUL2(a) mul2(a)	// mult 2
#define MUL3(a) (mul2(a))^a										// mult 3

typedef struct col_vec {
	unsigned char c[4];
} round_key;

static const unsigned char S_Box[256] =
	{
		
	};

static const unsigned char INV_SBOX[256] = {
	
};

// r = {1, X, X^2, ... X^7, X^8, X^9}
static unsigned char r[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

unsigned char mul2(unsigned char a);

#define MULe(a) (mul2(mul2(mul2(a))))^(mul2(mul2(a)))^(mul2(a))
#define MULb(a) (mul2(mul2(mul2(a))))^(mul2(a))^a
#define MULd(a) (mul2(mul2(mul2(a))))^(mul2(mul2(a)))^a
#define MUL9(a) (mul2(mul2(mul2(a))))^a

void print_key(unsigned char *a);
void genRoundKey(round_key *K, const unsigned char* a);

void subBytes(unsigned char *a);
void shiftRows(unsigned char *a);
void mixColumns(unsigned char *a);
void addRoundKey(round_key *K, unsigned char *a, int i);

void invAddRoundKey(round_key *K, unsigned char *a, int round);
void invMixColumns(unsigned char *a);
void invShiftRows(unsigned char *a);
void invSubBytes(unsigned char *a);

void _encrypt(unsigned char* a, round_key* K);
void _decrypt(unsigned char* a, round_key* K);

#endif //AES_H
